...
Mat Tracker::process(const Mat frame, Stats &stats) //SI se usa
{
    TickMeter tm;
    vector<KeyPoint> kp;
    Mat desc;
...
int main(int argc, char **argv)
{
    ...
    // Read image
    frame = imread("/home/mrh311/Pictures/Webcam/example1.jpg");

    // Select ROI
    vector<Point2f> bb;
    cv::Rect uBox = cv::selectROI(frame);
    bb.push_back(cv::Point2f(static_cast<float>(uBox.x), static_cast<float>(uBox.y)));
    bb.push_back(cv::Point2f(static_cast<float>(uBox.x + uBox.width), static_cast<float>(uBox.y)));
    bb.push_back(cv::Point2f(static_cast<float>(uBox.x + uBox.width), static_cast<float>(uBox.y + uBox.height)));
    bb.push_back(cv::Point2f(static_cast<float>(uBox.x), static_cast<float>(uBox.y + uBox.height)));
    ...
    akaze_tracker.setFirstFrame(frame, bb, "AKAZE", stats); //Frame es la primera imagen
    ...
    for (;;)
    {
        ...
        akaze_res = akaze_tracker.process(frame, stats); //Frame es el frame que se va capturando en tiempo real
        akaze_stats += stats;
        if (update_stats)
        {
